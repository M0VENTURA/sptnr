        ...
        for alb in albums:
            ...
            for t in tracks:
                ...
                td = {
                    ...
                    "navidrome_genres": ",".join([g.strip() for g in t.get("genre", "").replace("•", ",").replace(";", ",").replace("\\\\", ",").split(",") if g.strip()]) if t.get("genre") else "",  # Store as comma-separated string for template
