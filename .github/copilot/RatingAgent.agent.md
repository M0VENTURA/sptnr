...
description: >
  ...
  Navidrome/Subsonic). It must always commit changes to the /develop branch.
argument-hint: >
...
|--------|---------|-------------|-------|
| `lastfm.py` | Last.fm | `get_scrobbles()`, `get_listeners()`, `track_search()`, `get_similar_artists()` | API key required |
| `musicbrainz.py` | MusicBrainz | `search_release()`, `get_release_by_id()`, `get_artist_singles()` | No key; respect `Retry-After` header |
| `listenbrainz.py` | ListenBrainz | `get_listen_count()`, `get_user_count()`, `get_track_stats()` | No API key |
| `discogs.py` | Discogs | `search_release()`, `get_release()`, `check_single_video()` | Token required; 0.35 s/req rate limit |
| `audiodb.py` | AudioDB | `get_artist_biography()`, `get_artist_fanart()`, `get_album_info()` | Free API key `195003` |
| `navidrome.py` | Navidrome | `set_track_rating()`, `import_artists()`, `get_playlists()` | Subsonic API |
| `slskd.py` | Soulseek (slskd) | `search()`, `download()`, `get_status()` | Policy-gated in config |
| `coverartarchive.py` | Cover Art Archive | `get_album_art()` | No key required |
