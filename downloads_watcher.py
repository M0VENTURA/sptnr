...
def get_download_queue(status=None, limit=50, offset=0):
    ...
    Items with ``source='discovered'`` are auto-discovered filesystem files that
    are surfaced through the Downloads Folder Groups section of the UI.  They are
    excluded from ``total_count`` and the returned ``queue`` list (unless an
    explicit ``status`` filter is supplied) so that the pagination counter reflects
    only items the user explicitly added for download.  A separate
    ``discovered_count`` is included for informational purposes.
    """
    try:
        conn = get_db()
        ...
        if status:
            ...
            # Exclude auto-discovered filesystem files from the queue total so
            # the "Showing X of N" counter reflects only explicitly-queued items.
            cursor.execute("""
                SELECT COUNT(*) AS total
                FROM download_queue
                WHERE COALESCE(source, '') != 'discovered'
            """)
            total_count = int((cursor.fetchone() or {}).get('total', 0))
        ...
        # Separate count of auto-discovered folder files (for UI info / future use).
        discovered_count = 0
        if not status:
            cursor.execute("""
                SELECT COUNT(*) AS total
            ...
            """)
            discovered_count = int((cursor.fetchone() or {}).get('total', 0))
        ...
        if status:
            ...
            # Exclude auto-discovered items; they appear in the Folder Groups section.
            cursor.execute("""
                SELECT * FROM download_queue
                WHERE COALESCE(source, '') != 'discovered'
                ORDER BY
