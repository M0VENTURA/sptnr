...
def enrich_album(
    *,
    ...
    Returns a dict with enrichment results consumed downstream by track
    processing and finalisation stages.
    """
    ...
    try:
        ...
        # 3. Artist metadata (delegates to existing enrichment + metadata services)
        ...
        logger.error("[album_stage] Enrichment failed: %s", exc)
        try:
            conn.rollback()
        except Exception:
            pass
    ...
    # Roll up enrichment context for downstream stages
    extra_context = {}
    if meta.get("country"):
...
def _fetch_similar_artists(artist: str, conn, options: dict) -> dict[str, list]:
    ...
    singles_only = bool(options.get("singles_only")) or bool(options.get("singles_with_missing_popularity"))
    if singles_only:
        return result
    ...
    try:
        ...
        cached = cursor.fetchone()
        if cached:
            lastfm_raw = row_get(cached, "similar_artists_lastfm")
            lb_raw = row_get(cached, "similar_artists_listenbrainz")
            ts_raw = row_get(cached, "similar_artists_last_updated")
            if lastfm_raw:
                result["lastfm"] = json.loads(lastfm_raw)
            if lb_raw:
                result["listenbrainz"] = json.loads(lb_raw)
            # Use cache if < 90 days old
            if (result["lastfm"] or result["listenbrainz"]) and ts_raw:
                try:
        ...
        try:
            ...
            logger.debug("[album_stage] Last.fm similar artists failed for %s: %s", artist, exc)
        ...
        """, (
            artist, artist,
            json.dumps(result["lastfm"]) if result["lastfm"] else None,
            json.dumps(result["listenbrainz"]) if result["listenbrainz"] else None,
            datetime.now().isoformat(),
        ))
        conn.commit()
    except Exception as exc:
        logger.debug("[album_stage] Similar artists failed for %s: %s", artist, exc)
