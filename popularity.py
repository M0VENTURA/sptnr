...
def create_or_update_playlist_for_artist(artist_name: str, tracks: list):
    ...
    Logic:
      ...
      - Case C: if artist has any tracks (but doesn't meet A or B), create the playlist if it
                doesn't already exist (all tracks sorted by rating descending).
      - No tracks: delete any existing playlist.
    ...
    # CASE C - artist has tracks but doesn't meet A or B thresholds:
    ...
    if total_tracks > 0:
        ...
        if not os.path.exists(file_path):
            ...
            _create_nsp_file(playlist_name, playlist_data)
            log_basic(f"Essential playlist created for '{artist_name}' (all tracks by rating)")
        else:
            log_basic(f"Essential playlist already exists for '{artist_name}', skipping creation")
        return

    # No tracks at all: clean up any existing playlist
    log_basic(
